...
final class MessagesRawService implements MessagesRawContract
{
     ...
     * - A brand-new account can send on every channel immediately, but `sms`, `sms_oneway` and `voice` reach only the phone numbers the project has verified. Sending elsewhere returns `403` with code `destination_not_verified`; `details.verifiedNumbers` lists the numbers that are reachable. A number is verified from the dashboard's Sandbox screen: generate a code and send the pre-filled WhatsApp message from that phone to Zavu's sandbox number. One verification covers WhatsApp, SMS and calls, up to 5 numbers per project. To send to any destination, do any one of these: verify your identity, add a payment method, settle a deposit, or subscribe to a paid plan. Business verification (KYB) is required for **one channel only**: `sms_oneway`. Without an approved KYB, one-way SMS returns `403` with code `kyb_required` and `details.dashboardUrl` pointing at `/kyb`, whatever the account has otherwise verified. No other channel asks for it
